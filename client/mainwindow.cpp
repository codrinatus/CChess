    ...
    if (turn == true) {
        if (selected == false) {
           ...
           // if(clickedLabel->pixmap())
            ...
            if (old == clickedLabel) //nu permitem sa selectam aceeasi piesa de 2 ori
                selected = false;
            else {
                char move[10];
                if (white == true) {
...
void MainWindow::on_return2button_released()
{
...
void MainWindow::on_return1button_released()
{
    QProcess::startDetached(qApp->applicationFilePath(), QStringList());
    QCoreApplication::quit();
